...
std::vector<std::vector<IO_Flow_Parameter_Set*>*>* read_workload_definitions(const string workload_defs_file_path)
{
	...
	if (!workload_defs_file) {
		...
		string line((std::istreambuf_iterator<char>(workload_defs_file)), std::istreambuf_iterator<char>());//将文件流 workload_defs_file 中的全部内容读取并存储在字符串 line 中
		if (line.compare("USE_INTERNAL_PARAMS") != 0) {
...
int main(int argc, char* argv[])
{
	...
	int cntr = 1;
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {//这里是遍历每个trace
		time_t start_time = time(0);//表示代码执行的起始时间点
		char* dt = ctime(&start_time);//将时间戳转换为字符串格式
		...
		for (auto io_flow_def = (*io_scen)->begin(); io_flow_def != (*io_scen)->end(); io_flow_def++) {//遍历每个io流
			exec_params->Host_Configuration.IO_Flow_Definitions.push_back(*io_flow_def);//把io流相关的参数保存在IO_Flow_Definitions里
		}
		...
		exec_params->Host_Configuration.Input_file_path = workload_defs_file_path.substr(0, workload_defs_file_path.find_last_of("."));//Create Host_System based on the specified parameters,存储文件名
		...
		host.Attach_ssd_device(&ssd);//连接主机和ssd

		Simulator->Start_simulation();//Simulator就是Engine的指针
		...
		collect_results(ssd, host, (workload_defs_file_path.substr(0, workload_defs_file_path.find_last_of(".")) + "_scenario_" + std::to_string(cntr) + ".xml").c_str());//对每个trace都收集一次结果
	}
