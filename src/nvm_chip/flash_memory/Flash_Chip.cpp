...
namespace NVM
{
	namespace FlashMemory
	{
		...
		{
			int bits_per_cell = static_cast<int>(flash_technology);//每个存储单元存储的比特数
			_readLatency = new sim_time_type[bits_per_cell];//不同的页对应的读延迟和写延迟不一样（lsb、csb、msb）
		...
		{
			connectedReadyHandlers.push_back(function);//函数指针数组
		}
		...
		void Flash_Chip::Change_memory_status_preconditioning(const NVM_Memory_Address* address, const void* status_info)//状态信息吗？？
		{
		...
		void Flash_Chip::start_command_execution(Flash_Command* command)//开始命令执行
		{
			...
			if (command->Address.size() > 1
				&& (command->CommandCode == CMD_READ_PAGE
					...
					|| command->CommandCode == CMD_ERASE_BLOCK)) {//涉及到多个地址并且是一个普通的命令就要报错
			...
			targetDie->CommandFinishEvent = Simulator->Register_sim_event(targetDie->Expected_finish_time,
				this, command, static_cast<int>(Chip_Sim_Event_Type::COMMAND_FINISHED));//登记模拟事件
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)//完成命令执行
		{
			Die* targetDie = Dies[command->Address[0].DieID];//只会涉及一个晶圆吗?对
			...
			this->idleDieNo++;//完成命令执行了，则晶圆就变为空闲晶圆了
			if (idleDieNo == die_no) {//空闲晶圆数量和晶圆数量相同，代表芯片所有的晶圆都是空闲的
			...
			switch (command->CommandCode)
			{
				...
				{//擦除以块为粒度
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						STAT_eraseCount++;
						targetDie->Planes[command->Address[planeCntr].PlaneID]->Erase_count++;//分组的擦除次数，不记录块的擦除次数吗？
		...
		void Flash_Chip::Resume(flash_die_ID_type dieID)//暂停后恢复
		{
			Die* targetDie = Dies[dieID];
			if (!targetDie->Suspended) {
