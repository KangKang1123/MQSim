...
namespace MQSimEngine
{
	...
	///</summary>
	void EventTree::Add(sim_time_type key, Sim_Event* data)//给红黑树增加结点，按照key值决定新加结点位置
	{
		...
		while (temp != SentinelNode) {//找到结点所在位置的父节点
			// find Parent
			node->Parent = temp;
	...
	///</summary>
	void EventTree::RestoreAfterInsert(EventTreeNode* x)//恢复红黑树的特性
	{
	...
	{
		if (event->Fire_time < Engine::Instance()->Time()) {//大于等于才行，得在当前时间点之后
		...
		while (treeNode != SentinelNode)
		{
			if (key == treeNode->Key) {//同一时间点的直接加在链表后
		...
		Add(event->Fire_time, event);//不是同一时间点的会转化成结点插入红黑树
	}
