...
namespace Host_Components
{
	struct NVMe_Queue_Pair
	{
		...
		uint16_t Submission_queue_size;//提交队列大小
		uint64_t Submission_tail_register_address_on_device;//存储提交队列的尾部
		...
		uint64_t Completion_head_register_address_on_device;//存储完成队列的头部
		uint64_t Completion_queue_memory_base_address;
	};
//这个宏用于判断给定的 NVMe 提交队列是否已满，返回值为布尔类型（true 表示已满，false 表示未满）
...
#define NVME_UPDATE_SQ_TAIL(Q)  Q.Submission_queue_tail++;\
	...
	public:
		...
		virtual Host_IO_Request* Generate_next_request() = 0;//纯虚函数
	...
	protected:
		...
		std::vector<Host_IO_Request*> request_queue_in_memory;//内存中的请求队列
