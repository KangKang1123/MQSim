...
namespace SSD_Components
{
	...
	{
		capacity_in_pages = capacity_in_bytes / (SECTOR_SIZE_IN_BYTE * sector_no_per_page);//总容量的字节数/每页的字节数=页数量
	...
	void Data_Cache_Manager_Flash_Simple::process_new_user_request(User_Request* user_request)
	{
		...
		if (user_request->Type == UserRequestType::READ) {//读请求
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				case Caching_Mode::TURNED_OFF://关闭缓存模式，直接写入和读取实际存储介质
				...
				case Caching_Mode::WRITE_CACHE:
				{
					...
					while (it != user_request->Transaction_list.end()) {
						...
						if (data_cache->Exists(tr->Stream_id, tr->LPA)) {//缓存中找到事务对应的流和逻辑页地址所确定的key
							page_status_type available_sectors_bitmap = data_cache->Get_slot(tr->Stream_id, tr->LPA).State_bitmap_of_existing_sectors & tr->read_sectors_bitmap;//按位与运算，这个操作将会得到一个新的位图，其中包含了在缓存槽中存在的并且在读事务中也被请求的扇区
							if (available_sectors_bitmap == tr->read_sectors_bitmap) {//说明事务请求的扇区完全存在于缓存中
							...
							} else if (available_sectors_bitmap != 0) {//部分在缓存中
								user_request->Sectors_serviced_from_cache += count_sector_no_from_status_bitmap(available_sectors_bitmap);
								tr->read_sectors_bitmap = (tr->read_sectors_bitmap & ~available_sectors_bitmap);//清除已经从缓存中提供的扇区（先取反后与操作）
								tr->Data_and_metadata_size_in_byte -= count_sector_no_from_status_bitmap(available_sectors_bitmap) * SECTOR_SIZE_IN_BYTE;//减去已经从缓存中提供的扇区所占用的字节数
								it++;
							} else {
								it++;
							}
					...
					if (user_request->Sectors_serviced_from_cache > 0) {//从缓存中服务了部分用户请求数据
					...
					if (user_request->Transaction_list.size() > 0) {//事务列表中仍然包含未被服务的请求，这些事务不在或不全在缓存中可以处理
						static_cast<FTL*>(nvm_firmware)->Address_Mapping_Unit->Translate_lpa_to_ppa_and_dispatch(user_request->Transaction_list);
					}
				}
				default:
			...
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				{
					write_to_destage_buffer(user_request);//写入缓冲区
	...
	void Data_Cache_Manager_Flash_Simple::write_to_destage_buffer(User_Request* user_request)
	{
		...
		std::list<NVM_Transaction*>* evicted_cache_slots = new std::list<NVM_Transaction*>;//从缓存中驱逐的事务列表
		std::list<NVM_Transaction*> writeback_transactions;//需要写回到闪存的事务列表。
		...
		while (it != user_request->Transaction_list.end()
			...
			if (data_cache->Exists(tr->Stream_id, tr->LPA))//If the logical address already exists in the cache，缓存区中已经存在该写数据，更新缓存区中数据
			{
				...
				* This situation may result from out-of-order transaction execution*/
				...
				if (tr->DataTimeStamp > timestamp) {//写请求的数据时间戳大于缓存中的数据时间戳
					timestamp = tr->DataTimeStamp;
					content = tr->Content;
				}
				data_cache->Update_data(tr->Stream_id, tr->LPA, content, timestamp, tr->write_sectors_bitmap | slot.State_bitmap_of_existing_sectors);//不理解，为什么是进行或操作
			} else { //the logical address is not in the cache，插入到缓存中
				if (!data_cache->Check_free_slot_availability()) {//没有空闲缓存槽，去掉一个
			...
			if (bloom_filter[0].find(tr->LPA) == bloom_filter[0].end()) {//不在布隆过滤器中，bloom_filter是一个set的数组
				data_cache->Change_slot_status_to_writeback(tr->Stream_id, tr->LPA); //Eagerly write back cold data，把状态改为需要写回到闪存中
				flash_written_back_write_size_in_sectors += count_sector_no_from_status_bitmap(tr->write_sectors_bitmap);
				bloom_filter[0].insert(tr->LPA);//布隆过滤器存储热数据
				writeback_transactions.push_back(tr);//加入写回闪存的链表
			}
		...
		//If any writeback should be performed, then issue flash write transactions，写回到闪存中
		...
		if (Simulator->Time() > next_bloom_filter_reset_milestone) {//定期清理更新布隆过滤器，以确保其性能和准确性，不清理的话set容器会变得很大，而且之前用的很多数据现在很可能用不到了
	...
	void Data_Cache_Manager_Flash_Simple::service_dram_access_request(Memory_Transfer_Info* request_info)
	{
		...
		if(dram_execution_queue[request_info->Stream_id].size() == 1) {//说明dram当前是空闲的，可以处理该请求
