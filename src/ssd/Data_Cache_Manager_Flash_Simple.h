...
namespace SSD_Components
{
	...
	private:
		NVM_PHY_ONFI * flash_controller;//闪存控制器
		...
		unsigned int back_pressure_buffer_max_depth;//最大深度？
		unsigned int back_pressure_buffer_depth;//当前深度？
		std::set<LPA_type>* bloom_filter;//用于冷热分离
		sim_time_type bloom_filter_reset_step = 1000000000;
		sim_time_type next_bloom_filter_reset_milestone = 0;//下一个布隆过滤器重置的里程碑时间点
