...
public:
	...
	~Input_Stream_NVMe();
	IO_Flow_Priority_Class::Priority Priority_class;//io流对应的优先级
	...
	uint16_t Completion_tail;//可以表示队列中请求个数，队列是环形的
	...
	std::list<User_Request *> Completed_user_requests;		//The list of requests that are completed but have not been informed to the host due to full CQ,完成队列满所以未通知主机
	std::list<User_Request *> Waiting_write_data_transfers; //The list of write requests that are waiting for data
	uint16_t On_the_fly_requests;							// the number of requests that are either being fetch from host or waiting in the device queue,积压的请求数量
};
...
public:
	...
	unsigned int Queue_fetch_size;//从主机的io提交队列获取到设备级队列的最大io请求数
	...
	void Submission_queue_tail_pointer_update(stream_id_type stream_id, uint16_t tail_pointer_value);//提交队列从尾部添加
	void Completion_queue_head_pointer_update(stream_id_type stream_id, uint16_t head_pointer_value);//完成队列从头部删除
