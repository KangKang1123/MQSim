...
* as (64 - log_2(256)).
*/
#define LPN_TO_UNIQUE_KEY(STREAM,LPN) ((((LPA_type)STREAM)<<56)|LPN)//将流ID和逻辑页地址合并的唯一的键值，以便用作数据的索引或者唯一标识符
#define UNIQUE_KEY_TO_LPN(STREAM,LPN) ((~(((LPA_type)STREAM)<<56))&LPN)


inline unsigned int count_sector_no_from_status_bitmap(const page_status_type page_status)//计算扇区数量
{
	unsigned int size = 0;
	for (int i = 0; i < 64; i++) {//从0到63遍历页面状态位图的每一个位
		if ((((page_status_type)1) << i) & page_status) {//相当于将其中的"1"位移到第i位，做与操作可以判断'page_status'中相应位置上的位是否为1
			size++;//结果非0，代表这个扇区存在
		}
	}
