...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	{
		PPA_type PPA;
		unsigned long long WrittenStateBitmap;//记录该页哪些扇区已被写入
		bool Dirty;
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		...
		* proposal. It is always stored in non-volatile flash memory.*/
		GMTEntryType* GlobalMappingTable;//缓存中只存储最常使用的映射，其余映射保存在闪存中
		...
		std::multimap<MVPN_type, LPA_type> ArrivingMappingEntries;//正在到达的映射项
		std::set<MVPN_type> DepartingMappingEntries;//正在离开的映射项
	...
	public:
		...
		void Store_mapping_table_on_flash_at_start();//将映射表存储到闪存中
