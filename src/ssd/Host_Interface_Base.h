...
{
#define COPYDATA(DEST,SRC,SIZE) if(Simulator->Is_integrated_execution_mode()) {DEST = new char[SIZE]; memcpy(DEST, SRC, SIZE);} else DEST = SRC;//执行深拷贝和浅拷贝的区别
#define DELETE_REQUEST_NVME(REQ) \
	...
	protected:
		enum class DMA_Req_Type { REQUEST_INFO, WRITE_DATA };//用于请求信息和用于写数据的请求
		struct DMA_Req_Item
		{
			DMA_Req_Type Type;
			void * object;
	...
	protected:
		...
		void broadcast_user_request_arrival_signal(User_Request* user_request)
		{
			for (std::vector<UserRequestArrivedSignalHandlerType>::iterator it = connected_user_request_arrived_signal_handlers.begin();
				it != connected_user_request_arrived_signal_handlers.end(); it++) {//函数数组里的每个函数都处理一遍这个函数
				(*it)(user_request);
			}
		}
