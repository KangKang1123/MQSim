...
namespace SSD_Components
{
	...
	Data_Cache_Slot_Type Data_Cache_Flash::Get_slot(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (lru_list.begin()->first != key) {
			lru_list.splice(lru_list.begin(), lru_list, it->second->lru_list_ptr);//LRU,把当前缓存槽移动到LRU列表最前面
		}

		return *(it->second);
	}

	bool Data_Cache_Flash::Check_free_slot_availability()//检测是否有空闲缓存槽可用
	{
		return slots.size() < capacity_in_pages;
	}

	bool Data_Cache_Flash::Check_free_slot_availability(unsigned int no_of_slots)//检测是否有指定数量的缓存槽
	{
	...
	{
		assert(slots.size() > 0);
		auto itr = lru_list.rbegin();//最近最少使用的缓存槽
		while (itr != lru_list.rend()) {
			if ((*itr).second->Status == Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK) {//找到一个脏缓存槽，且数据没有写入存储介质中
				break;
			}
			itr++;
		}
		...
		if (itr == lru_list.rend()) {//说明未找到脏缓存槽
			evicted_item.Status = Cache_Slot_Status::EMPTY;
			return evicted_item;//返回一个状态为"EMPTY"的缓存槽
		}

		slots.erase(lru_list.back().first);//不太懂，怎么变成删除最后一个了
	...
	void Data_Cache_Flash::Insert_read_data(const stream_id_type stream_id, const LPA_type lpn, const data_cache_content_type content,
		const data_timestamp_type timestamp, const page_status_type state_bitmap_of_read_sectors)//将读取的数据信息插入到缓存中
	{
		LPA_type key = LPN_TO_UNIQUE_KEY(stream_id, lpn);
		...
		const data_timestamp_type timestamp, const page_status_type state_bitmap_of_write_sectors)
	{
		...
		cache_slot->Content = content;
		cache_slot->Timestamp = timestamp;
		cache_slot->Status = Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK;//数据还没写回存储介质
		lru_list.push_front(std::pair<LPA_type, Data_Cache_Slot_Type*>(key, cache_slot));
		cache_slot->lru_list_ptr = lru_list.begin();
		slots[key] = cache_slot;
	}
		...
		const data_timestamp_type timestamp, const page_status_type state_bitmap_of_write_sectors)
	{
		...
		it->second->Status = Cache_Slot_Status::DIRTY_NO_FLASH_WRITEBACK;//在缓存中更新数据，肯定是写数据，还没写回存储介质
		if (lru_list.begin()->first != key) {//把当前缓存槽放在最前面
