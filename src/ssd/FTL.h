...
namespace SSD_Components
{
	...
	public:
		...
		~FTL();
		void Perform_precondition(std::vector<Utils::Workload_Statistics*> workload_stats);//执行预处理的函数，也就是稳态
		...
		Address_Mapping_Unit_Base* Address_Mapping_Unit;//地址映射单元
	...
	private:
		...
		double over_provisioning_ratio;//为了提高性能和可靠性，通常会保留一部分存储容量不对外部用户开放，而是用于内部管理、坏块处理、擦除等操作
