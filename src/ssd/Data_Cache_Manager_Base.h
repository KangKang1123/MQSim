...
namespace SSD_Components
{
	...
	enum class Cache_Sharing_Mode { SHARED,//each application has access to the entire cache space;一种是每个应用程序都可以访问整个缓存空间，一种是每个应用程序只能访问自己被分配的缓存部分
		EQUAL_PARTITIONING}; 
	...
	public:
		...
		typedef void(*UserRequestServicedSignalHanderType) (User_Request*);//函数指针
		void Connect_to_user_request_serviced_signal(UserRequestServicedSignalHanderType);//参数是一个函数指针，把函数放入函数指针数组中
		typedef void(*MemoryTransactionServicedSignalHanderType) (NVM_Transaction*);//函数指针
		void Connect_to_user_memory_transaction_serviced_signal(MemoryTransactionServicedSignalHanderType);//参数是一个函数指针，把函数放入函数指针数组中
	...
	protected:
		...
		void broadcast_user_request_serviced_signal(User_Request* user_request);//执行上面数组里的每一个函数

		std::vector<MemoryTransactionServicedSignalHanderType> connected_user_memory_transaction_serviced_signal_handlers;
		void broadcast_user_memory_transaction_serviced_signal(NVM_Transaction* transaction);//执行上面数组里的每一个函数
