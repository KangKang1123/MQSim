...
inline void Input_Stream_Manager_NVMe::Handle_new_arrived_request(User_Request *request)
{
	...
	{ //Circular queue implementation，循环队列
	...
	if (request->Type == UserRequestType::READ)
	{
		...
		((Host_Interface_NVMe *)host_interface)->broadcast_user_request_arrival_signal(request);//用户请求到达信号
	}
	else
	{ //This is a write request
		((Input_Stream_NVMe *)input_streams[request->Stream_id])->Waiting_user_requests.push_back(request);
		((Input_Stream_NVMe *)input_streams[request->Stream_id])->STAT_number_of_write_requests++;
		((Host_Interface_NVMe *)host_interface)->request_fetch_unit->Fetch_write_data(request);//获取写请求的数据
	}
}
...
inline void Input_Stream_Manager_NVMe::Handle_serviced_request(User_Request *request)//处理已完成的用户请求
{
	...
	if (((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head != ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_tail)//表示提交队列中还有等待中的请求
	{
		((Host_Interface_NVMe *)host_interface)->request_fetch_unit->Fetch_next_request(stream_id);
		((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests++;
		((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head++; //Update submission queue head after starting fetch request
	...
	if (((Input_Stream_NVMe *)input_streams[stream_id])->Completion_head > ((Input_Stream_NVMe *)input_streams[stream_id])->Completion_tail)
	{
		...
		if (((Input_Stream_NVMe *)input_streams[stream_id])->Completion_tail + 1 == ((Input_Stream_NVMe *)input_streams[stream_id])->Completion_head)//不明白为什么是尾部+1==头部,环状
		{
	...
	inform_host_request_completed(stream_id, request); //Completion queue is not full, so the device can DMA the completion queue entry to the host，完成队列不满就可以直接给主机，完成队列满请求就得放在队列中等待
...
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)//给用户请求分割成多个事务
{
	LHA_type lsa = user_request->Start_LBA;
	LHA_type lsa2 = user_request->Start_LBA;
	unsigned int req_size = user_request->SizeInSectors;//用户请求大小（扇区个数）

	page_status_type access_status_bitmap = 0;
	unsigned int handled_sectors_count = 0;//已处理的扇区个数
	...
	while (handled_sectors_count < req_size)
	{
		...
		LHA_type internal_lsa = lsa - ((Input_Stream_NVMe *)input_streams[user_request->Stream_id])->Start_logical_sector_address; //For each flow, all lsa's should be translated into a range starting from zero，内部偏移量

		transaction_size = host_interface->sectors_per_page - (unsigned int)(lsa % host_interface->sectors_per_page);//该值不是sectors_per_page就是小于这个数的值，表示本次循环要处理的扇区个数
		if (handled_sectors_count + transaction_size >= req_size)//说明到最后一页了
		{
			transaction_size = req_size - handled_sectors_count;
		}
		LPA_type lpa = internal_lsa / host_interface->sectors_per_page;//逻辑页地址
		...
		{ //user_request->Type == UserRequestType::WRITE
			...
			user_request->Transaction_list.push_back(transaction);//把事务放入请求的事务链表里
			input_streams[user_request->Stream_id]->STAT_number_of_write_transactions++;
		}

		lsa = lsa + transaction_size;//下一批扇区的起始地址
		handled_sectors_count += transaction_size;//已处理的扇区数量
	}
}
...
void Request_Fetch_Unit_NVMe::Fetch_next_request(stream_id_type stream_id)
{
	...
	host_interface->Send_read_message_to_host(im->Submission_queue_base_address + im->Submission_head * sizeof(Submission_Queue_Entry), sizeof(Submission_Queue_Entry));//将请求发送给主机获取下一个请求
}
