...
namespace SSD_Components
{
		...
		mapping_table_stored_on_flash(false)
	{
		...
		total_logical_pages_no = (unsigned int)((double)total_physical_pages_no * (1 - overprovisioning_ratio));//逻辑页数量，相当于和物理页是一对一关系
		max_logical_sector_address = (LHA_type)(SectorsPerPage * total_logical_pages_no - 1);//每个页面的扇区数在哪
	}
