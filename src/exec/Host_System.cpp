...
Host_System::Host_System(Host_Parameter_Set* parameters, bool preconditioning_required, SSD_Components::Host_Interface_Base* ssd_host_interface)://ssd_host_interface指向的是子类对象
	MQSimEngine::Sim_Object("Host"), preconditioning_required(preconditioning_required)//id为“Host”
{
	Simulator->AddObject(this);

	//Create the main components of the host system
	if (((SSD_Components::Host_Interface_NVMe*)ssd_host_interface)->GetType() == HostInterface_Types::SATA) {//这里调用的是基类的函数,注意这里的括号很重要
	...
	for (uint16_t flow_id = 0; flow_id < parameters->IO_Flow_Definitions.size(); flow_id++) {//遍历每个io流
		...
		uint16_t nvme_sq_size = 0, nvme_cq_size = 0;//sq是提交队列，cq是完成队列
		switch (((SSD_Components::Host_Interface_NVMe*)ssd_host_interface)->GetType()) {
			case HostInterface_Types::NVME:
				nvme_sq_size = ((SSD_Components::Host_Interface_NVMe*)ssd_host_interface)->Get_submission_queue_depth();//这里的队列大小是从哪传过来的？？创建派生类对象的时候就赋值了
		...
		switch (parameters->IO_Flow_Definitions[flow_id]->Type) {
			...
			case Flow_Type::TRACE: {
				IO_Flow_Parameter_Set_Trace_Based * flow_param = (IO_Flow_Parameter_Set_Trace_Based*)parameters->IO_Flow_Definitions[flow_id];//IO_Flow_Definitions[flow_id]是IO_Flow_Parameter_Set*类型
                //下面是多态，基类指针指向子类对象，io_flow是IO_Flow_Base*类型
				...
				this->IO_flows.push_back(io_flow);//std::vector<Host_Components::IO_Flow_Base*> IO_flows;
				break;
			}
			default:
...
void Host_System::Validate_simulation_config() //验证模拟配置
{
