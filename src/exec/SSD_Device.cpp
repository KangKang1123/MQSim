...
SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows) : MQSimEngine::Sim_Object("SSDDevice")
{
	...
	{
		sim_time_type *read_latencies, *write_latencies;//uint64_t 表示无符号 64 位整数。
		...
		case Flash_Technology_Type::SLC:
			read_latencies = new sim_time_type[1];//读延迟，创建了一个数组，数组空间在堆中分配，SLC一个单元存储1bit数据
		...
		this->Channel_count = parameters->Flash_Channel_Count;//闪存通道数量
		this->Chip_no_per_channel = parameters->Chip_No_Per_Channel;//每个通道对应的芯片数量
		switch (parameters->Flash_Comm_Protocol)
		{
		case SSD_Components::ONFI_Protocol::NVDDR2://协议
		{
			SSD_Components::ONFI_Channel_NVDDR2 **channels = new SSD_Components::ONFI_Channel_NVDDR2 *[parameters->Flash_Channel_Count];//通道
			for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)//counter计数器
			{
				NVM::FlashMemory::Flash_Chip **chips = new NVM::FlashMemory::Flash_Chip *[parameters->Chip_No_Per_Channel];//芯片
		...
		device->Firmware = ftl;//派生类指针赋值给基类指针

		//Step 5: create TSU
		SSD_Components::TSU_Base *tsu;
		bool erase_suspension = false, program_suspension = false;//闪存芯片支持的挂起命令的类型，擦除挂起、编程挂起
		...
		case SSD_Components::Flash_Scheduling_Type::PRIORITY_OUT_OF_ORDER:
			tsu = new SSD_Components::TSU_Priority_OutOfOrder(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2 *>(device->PHY),
										  ...
										  parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read,
										  parameters->Preferred_suspend_erase_time_for_write,
										  erase_suspension, program_suspension);//static_cast为强制类型转换，这里是把基类指针转化为派生类指针，这里可能有风险
			break;
		...
		std::vector<std::vector<flash_channel_ID_type>> flow_channel_id_assignments;//给每个io流分配相应的通道
		std::vector<std::vector<flash_chip_ID_type>> flow_chip_id_assignments;//芯片分配
		std::vector<std::vector<flash_die_ID_type>> flow_die_id_assignments;//晶圆分配
		std::vector<std::vector<flash_plane_ID_type>> flow_plane_id_assignments;//分组分配
		unsigned int stream_count = 0;
		for (unsigned int i = 0; i < io_flows->size(); i++)
		{
			switch (parameters->HostInterface_Type)
			{
			case HostInterface_Types::SATA://SATA协议中每个io流都可以使用所有的资源，不能把io流分开分配
			{
				stream_count = 1;
			...
			{
				stream_count = (unsigned int)io_flows->size();//io流数量
				std::vector<flash_channel_ID_type> channel_ids;
				flow_channel_id_assignments.push_back(channel_ids);
		...
		for (unsigned int i = 0; i < io_flows->size(); i++)
		{
			...
			{
				max_rho = (*io_flows)[i]->Initial_Occupancy_Percentage;//预处理时写入逻辑空间的百分比
			}
		}
		...
		//数据缓存机制，分为简单和高级，后面实现新的缓存机制可以在这里加
		...
		ftl->Data_cache_manager = dcm;//为什么要在两个地方都搞？？是代表既属于设备也属于ftl吗
